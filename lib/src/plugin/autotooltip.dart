...
class AutoTooltips extends IPlugin {
  ...
  init(grid) {
    ...
    _grid = grid;
    if (_options['enableForCells'])
      _grid.onMouseEnter.subscribe(handleMouseEnter);
    if (_options['enableForHeaderCells'])
  ...
  handleMouseEnter(core.EventData e, [Map arg]) {
    ...
    if (cell != null) {
      ...
      if ($node.paddingEdge.width < $node.scrollWidth) {
        text = $node.text;
        if (_options['maxToolTipLength'] != null &&
            text.length > _options['maxToolTipLength']) {
  ...
  handleHeaderMouseEnter(core.EventData e, Map args) {
    var column = args['column'],
        $node = findClosestAncestor(e.target, ".slick-header-column");
    if (column['toolTip'] == null) {
      //when tool tip is not defined
      $node.attributes["title"] =
          $node.paddingEdge.width < $node.scrollWidth ? column.name : "";
    }
  }
}
