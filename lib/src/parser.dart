...
Logger _log = new Logger('slick.parser');
...
//  Map m;
  ...
  CsvAdapter(String csv, [this.charWidth = 8, this.widthBase = 10]) {
    ...
    _strs
        .sublist(1, _strs.length > 10 ? 10 : _strs.length)
        .forEach((String line) => _updateMaxLen(line.split(",")));
  ...
  List<Map> makeData(List<String> strs) {
    return strs
        .sublist(1)
        .map((String line) => _toDataMap(line.split(',')))
        .toList();
  }
